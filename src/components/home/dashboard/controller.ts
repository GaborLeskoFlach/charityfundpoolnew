...
import {  IUserMapping, ILocation, RegistrationType } from '../../interfaces';
                ...
                if(user && user.locations){
                    user.locations.sort((a,b) => { return new Date(a.createdAt).getDate() - new Date(b.createdAt).getDate()}).map((location : ILocation) => {
                        ...
                        switch(location.registrationType){
                            ...
                            regType = { displayText : 'Needs help (Individual)', redirectLink : '/register/NeedHelp/individual' }
                            break
                            case RegistrationType.NeedHelpOrg:
                            regType = { displayText : 'Needs help (Organisation)', redirectLink : '/register/NeedHelp/organisation' }
                            break
